...
public:
    ...
    void set_coloring_strategy(const std::function<ImU32(const T &)> &coloring_strategy);

    void add_on_node_hover(std::function<void(const T &)> callback) { node_hover_cbs_.push_back(callback); }
    void add_on_node_click(std::function<void(const T &)> callback) { node_clicked_cbs_.push_back(callback); }
...
private:
    ...
    void squarify_layout(std::vector<int> &indices, float x, float y, float width, float height);
    float calculate_aspect_ratio(const std::vector<int> &indices, float side);
    void place_rectangles(const std::vector<int> &indices, float x, float y, float width, float height, bool horizontal);
};
...
template <TreeNode T>
void TreeMapWidget<T>::set_coloring_strategy(const std::function<ImU32(const T &)> &coloring_strategy)
{
...
template <TreeNode T>
void TreeMapWidget<T>::flatten_tree(const T &node)
{
...
template <TreeNode T>
void TreeMapWidget<T>::squarify_layout(std::vector<int> &indices, float x, float y, float width, float height)
{
    if (indices.empty())
        return;
    ...
    if (!remaining.empty())
    {
        ...
        if (horizontal)
        {
            squarify_layout(remaining, x, y + row_thickness, width, height - row_thickness);
        }
        else
        {
            squarify_layout(remaining, x + row_thickness, y, width - row_thickness, height);
        }
    }
}

template <TreeNode T>
float TreeMapWidget<T>::calculate_aspect_ratio(const std::vector<int> &indices, float side)
{
    if (indices.empty())
        return 1.0f;
...
template <TreeNode T>
void TreeMapWidget<T>::place_rectangles(const std::vector<int> &indices, float x, float y, float width, float height, bool horizontal)
{
    if (indices.empty())
        return;
    ...
    for (int idx : indices)
    {
        ...
        if (horizontal)
        {
            ...
            rendered_rects_.emplace_back(node, x + current_offset, y, node_width, height);
            current_offset += node_width;
        }
        else
        {
            float node_height = node_area / width;
            rendered_rects_.emplace_back(node, x, y + current_offset, width, node_height);
...
bool TreeMapWidget<T>::Render(const char *label, const ImVec2 &size)
{
    ...
    for (auto &rect : rendered_rects_)
    {
        ...
        {
            draw_list->AddRectFilled(rect_min, rect_max, IM_COL32(255, 255, 0, 100));
        }

        draw_list->AddRectFilled(rect_min, rect_max, fill_color);
        draw_list->AddRect(rect_min, rect_max, IM_COL32(255, 255, 255, 180), 0.0f, 0, 1.5f);
    }
